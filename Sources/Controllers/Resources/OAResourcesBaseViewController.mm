...
- (void) offerDownloadAndInstallOf:(OARepositoryResourceItem *)item
{
    ...
    if (isWeatherForecast && [OAWeatherHelper hasStatus:EOAWeatherForecastStatusCalculating regionId:item.worldRegion.regionId])
        return;
...
- (void) offerDownloadAndUpdateOf:(OAOutdatedResourceItem *)item
{
    ...
    {
        if ([OAWeatherHelper hasStatus:EOAWeatherForecastStatusCalculating regionId:item.worldRegion.regionId])
            return;
...
- (void) onItemClicked:(id)senderItem
{
    if ([senderItem isKindOfClass:[OAResourceItem class]] || [senderItem isKindOfClass:[OASearchResult class]])
    {
        ...
        else if (item_.resourceType == OsmAndResourceType::WeatherForecast && [OAWeatherHelper hasStatus:EOAWeatherForecastStatusDownloading regionId:item_.worldRegion.regionId])
        {
