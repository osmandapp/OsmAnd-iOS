...
#define DISTANCE_TO_REFRESH 25000
...
- (BOOL) updateInfo
{
    if (self.shouldUpdate)
    {
        ...
        if (!externalMaps.empty())
        {
            ...
            [self updateVisibility:self.isVisible];
        }
        else
        {
...
- (void) updateWidgetInformation
{
    if (_resourceItem)
    {
        ...
        [attrString addAttribute:NSForegroundColorAttributeName value:UIColorFromRGB(_nightMode ? color_chart_orange : color_primary_purple) range:range];
...
- (BOOL) shouldUpdate
{
    if (![_settings.showDownloadMapDialog get] && self.isHidden)
        return NO;
    ...
    {
        if ([_cachedLocation distanceFromLocation:currentLocation] > DISTANCE_TO_REFRESH)
        {
            _cachedLocation = currentLocation;
            return YES;
        }
...
{
    if (_resourceItem)
        [OAResourcesUIHelper offerDownloadAndInstallOf:_resourceItem onTaskCreated:nil onTaskResumed:nil];
    [self updateInfo];
}
