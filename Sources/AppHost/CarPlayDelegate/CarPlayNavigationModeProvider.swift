//
//  CarPlayNavigationModeProvider.swift
//  OsmAnd Maps
//
...
final class CarPlayNavigationModeProvider {    
    // App mode (profile) that was active before switching to a CarPlay mode.
    private(set) var originalAppMode: OAApplicationMode?
    ...
    func firstCarMode() -> OAApplicationMode {
        guard
            ...
            NSLog("[CarPlayNavigationModeProvider] -> No car-derived mode found. Fallback = %@", fallback.toHumanString())
            return fallback
        }
