...
- (void) generateData
{
    ...
    [data addObject:
     @{
         ...
         @"distance" : gpxData ? [formatter getFormattedDistance:gpxData.totalDistance] : @"",
         @"time" : gpxData ? [formatter getFormattedTimeInterval:gpxData.timeSpan shortFormat:YES] : @"",
    ...
    for (OAGpxTrkSeg *seg in [_gpx getNonEmptyTrkSegments:NO])
    {
        ...
        item[@"distance"] = [formatter getFormattedDistance:segmentDist];
        
        if (segmentTime != 1)
            item[@"time"] = [formatter getFormattedTimeInterval:segmentTime shortFormat:YES];
