//
//  OACoordinatesMapCenterWidget.m
//  OsmAnd Maps
//
//  Created by nnngrach on 06.03.2023.
...
@interface OACoordinatesMapCenterWidget ()
...
@implementation OACoordinatesMapCenterWidget
{
...
- (instancetype) init
{
    NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"OACoordinatesMapCenterWidget" owner:self options:nil];
    self = (OACoordinatesMapCenterWidget *)[nib objectAtIndex:0];
    if (self)
...
- (BOOL) isVisible
{
    ...
    return [_settings.showMapCenterCoordinatesWidget get]
...
- (BOOL) updateInfo
{
    ...
    if ([self shouldUpdate])
    {
        ...
        _cachedVisibiliy = [_settings.showMapCenterCoordinatesWidget get];
        ...
        if (visible)
        {
            ...
            _lastKnownLocation = [[CLLocation alloc] initWithLatitude:latLon.latitude longitude:latLon.longitude];
            ...
            if (_lastKnownLocation)
            {
                ...
                if (format == MAP_GEO_UTM_FORMAT)
                {
                    ...
                    [_latImageView setImage:[UIImage imageNamed:nightMode ? @"widget_coordinates_map_center_utm_night" : @"widget_coordinates_map_center_utm_day"]];
                ...
                else
                {
                    ...
                    NSString* latDayImg = lat >= 0 ? @"widget_coordinates_map_center_latitude_north_day" : @"widget_coordinates_map_center_latitude_south_day";
                    NSString* latNightImg = lat >= 0 ? @"widget_coordinates_map_center_latitude_north_night" : @"widget_coordinates_map_center_latitude_south_night";
                    NSString* lonDayImg = lon >= 0 ? @"widget_coordinates_map_center_longitude_east_day" : @"widget_coordinates_map_center_longitude_west_day";
                    NSString* lonNightImg = lon >= 0 ? @"widget_coordinates_map_center_longitude_east_night" : @"widget_coordinates_map_center_longitude_west_night";
...
- (BOOL) shouldUpdate
{
    ...
    {
        BOOL isVisibilityChanged = _cachedVisibiliy != [_settings.showMapCenterCoordinatesWidget get];
