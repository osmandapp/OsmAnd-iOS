//
//  GeoNavigationAction.swift
//  OsmAnd
//
//  Created by Oleksandr Panchenko on 15.04.2026.
...
struct GeoNavigationParser {
    ...
    static func parse(_ url: URL) -> GeoNavigationAction? {
        ...
        let params = Dictionary(uniqueKeysWithValues: queryItems.map { ($0.name.lowercased(), $0.value ?? "") })
        ...
        if path.contains("directions") {
            guard let destValue = params["destination"] else { return nil }
            
            if let destCoord = parseCoord(destValue) {
                let srcCoord = params["source"].flatMap { parseCoord($0) }
                return .buildRoute(source: srcCoord, destination: destCoord, waypoints: waypoints)
            } else {
                return .search(query: destValue)
            }
        }
