...
- (void)onQuickActionButtonDragged:(UILongPressGestureRecognizer *)recognizer
{
    if ([recognizer.view isKindOfClass:OAHudButton.class])
    {
        ...
        if (quickActionButton.buttonState && [quickActionButton.buttonState isKindOfClass:QuickActionButtonState.class])
            [self onButtonDragged:recognizer button:quickActionButton];
    }
}

- (void)onMap3dModeButtonDragged:(UILongPressGestureRecognizer *)recognizer
{
    [self onButtonDragged:recognizer button:_map3dModeFloatingButton];
}

- (void)onButtonDragged:(UILongPressGestureRecognizer *)recognizer button:(OAHudButton *)button
{
    UIView *hostView = button.superview ?: self.view;
    if (!hostView)
        return;
            ...
            else
            {
                for (OAHudButton *button in _quickActionFloatingButtons)
                {
                    if ([button.buttonState isKindOfClass:QuickActionButtonState.class])
                    {
                        ...
                        {
                            [_mapHudController.mapHudLayout removeMapButton:button];
...
- (void)adjustMapViewPort
{
    ...
    [mapPanel.mapViewController setViewportScaleX:isLandscape ? kViewportBottomScale : kViewportScale y:isLandscape ? kViewportScale : 2 * (height - _actionsView.frame.size.height) / height];
}
...
- (void)restoreControlsPosition 
{
    ...
    [_mapHudController.mapHudLayout onContainerSizeChanged];
    [self restorePinPosition];
}
...
- (void)showActionsSheetAnimated:(OAHudButton *)quickActionButton
{
    ...
    [self restorePinPosition];
    [[OARootViewController instance].mapPanel.mapViewController cancelAutoZoomAnimation];
