...
- (NSAttributedString *)getAttributedTypeStr:(NSString *)group color:(nullable UIColor *)color
{
    ...
    groupAttachment.image = [OAUtilities tintImageWithColor:[UIImage imageNamed:@"map_small_group"] color:iconColor];
...
- (void)setMiddleToolbarAlpha:(CGFloat)alpha
{
    if ([self hasTopToolbar])
    {
        ...
        if (![OAUtilities isLandscape] && ![OAUtilities isIPad])
            [OARootViewController.instance.mapPanel.hudViewController updateTopControlsVisibility:YES];
