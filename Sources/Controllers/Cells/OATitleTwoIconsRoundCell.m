...
- (void)highlight:(BOOL)highlighted
{
    if (highlighted)
    {
        ...
        [_leftIconView setTintColor:UIColor.whiteColor];
    }
    else
    {
        ...
        [_leftIconView setTintColor:_iconColorNormal ? _iconColorNormal : UIColorFromRGB(color_primary_purple)];
    }
}
...
- (void) applyCornerRadius
{
    ...
    CGRect iconFrame = self.leftIconView.frame;
    iconFrame.origin.x = cellMargin;
    iconFrame.origin.y = _contentContainer.frame.size.height / 2 - iconFrame.size.height / 2;
    self.leftIconView.frame = iconFrame;
    
    CGFloat textX = 2*cellMargin + iconFrame.size.width;
