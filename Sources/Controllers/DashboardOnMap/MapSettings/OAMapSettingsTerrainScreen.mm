    ...
    else if ([item.cellType isEqualToString:[OAButtonTableViewCell getCellIdentifier]])
    {
        ...
        if (cell)
        {
            ...
            else
            {
                ...
                conf.title = nil;
                conf.image = nil;
                conf.background.image = [UIImage imageNamed:item.secondaryIconName];
                conf.background.imageContentMode = UIViewContentModeScaleAspectFill;
                conf.background.cornerRadius = 8.0;
                conf.contentInsets = NSDirectionalEdgeInsetsMake(16, 35, 16, 35);
