...
- (id) initWithRenderedObject:(OARenderedObject *)renderedObject
{
    ...
    OAPOIType *otherPt;
    NSString *subtype;
    NSMutableDictionary<NSString *, NSString *> *additionalInfo = [NSMutableDictionary new];
    NSMutableDictionary<NSString *, NSString *> *localizedNames = [NSMutableDictionary new];
    
    for (NSString *tag in renderedObject.tags.allKeys)
    {
        NSString *value = renderedObject.tags[tag];
        ...
        {
            NSString *lang = [tag substringToIndex:@"name:".length];
            localizedNames[lang] = value;
            continue;
        }
        if ([tag isEqualToString:@"amenity"])
        {
            if (pt)
                otherPt = pt;
            pt = [[OAPOIHelper sharedInstance] getPoiTypeByKey:value];
        }
        else
        {
            NSString *key = [NSString stringWithFormat:@"%@_%@", tag, value];
            OAPOIType *poiType = [[OAPOIHelper sharedInstance] getPoiTypeByKey:key];
            if (poiType)
            {
                otherPt = pt ? poiType : otherPt;
                subtype = !pt ? value : subtype;
                pt = !pt ? poiType : pt;
            }
        }
        if (value.length == 0 && !otherPt)
        {
            otherPt = [[OAPOIHelper sharedInstance] getPoiTypeByKey:tag];
        }
        if (!otherPt)
        {
            OAPOIType *poiType  = [[OAPOIHelper sharedInstance] getPoiTypeByKey:tag];
            if (poiType && [poiType.getOsmTag isEqualToString:tag])
                otherPt = poiType;
        }
        if (value.length != 0)
        {
             ...
             */
            NSString *translate = [[OAPOIHelper sharedInstance] getTranslation:[NSString stringWithFormat:@"%@_%@", tag, value]];
            NSString *translate2 = [[OAPOIHelper sharedInstance] getTranslation:value];
    ...
    [poi setX:renderedObject.x];
    [poi setY:renderedObject.y];
