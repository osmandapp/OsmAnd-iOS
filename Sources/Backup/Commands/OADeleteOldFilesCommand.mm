...
@implementation OADeleteOldFilesCommand
{
...
- (void)doInBackground
{
    ...
    OAOperationLog *operationLog = [[OAOperationLog alloc] initWithOperationName:@"deleteOldFileList" debug:BACKUP_DEBUG_LOGS];
    ...
    [OANetworkUtilities sendRequestWithUrl:OABackupHelper.LIST_FILES_URL params:params post:NO async:NO onComplete:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        else if (result.length > 0)
        {
            ...
            if (!jsonParsingError)
            {
                ...
                NSArray *uniqueFiles = resultJson[@"uniqueFiles"];
                for (NSDictionary *uniqueFile in uniqueFiles)
                {
                    [remoteFiles removeObject:[[OARemoteFile alloc] initWithJson:uniqueFile]];
                }
