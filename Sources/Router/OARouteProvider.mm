...
- (OAGPXRouteParams *) prepareGPXFile:(OAGPXRouteParamsBuilder *)builder
{
    ...
    if (_file.getPointsList.count > 0)
    {
        self.wpt = [NSArray arrayWithArray:_file.getPointsList];
    }
    NSInteger selectedSegment = builder.selectedSegment;
    if ([OSMAND_ROUTER_V2 isEqualToString:_file.author])
    {
        ...
        _route = [OARouteProvider parseOsmAndGPXRoute:points gpxFile:_file segmentEndpoints:endPoints selectedSegment:selectedSegment leftSide:builder.leftSide];
        _points = points;
        _segmentEndPoints = endPoints;
        
        if (selectedSegment == -1)
            _routePoints = [_file getRoutePoints];
        else
            _routePoints = [_file getRoutePointsRouteIndex:(int)selectedSegment];
    ...
    else if ([_file isCloudmadeRouteFile] || [OSMAND_ROUTER isEqualToString:_file.author])
    {
        ...
        self.directions = [OARouteProvider parseOsmAndGPXRoute:points gpxFile:_file segmentEndPoints:endPoints osmandRouter:[OSMAND_ROUTER isEqualToString:_file.author] leftSide:builder.leftSide defSpeed:10 selectedSegment:selectedSegment];
        self.points = [NSArray arrayWithArray:points];
        _segmentEndPoints = endPoints;
        if ([OSMAND_ROUTER isEqualToString:_file.author])
        {
    ...
    else
    {
        ...
        {
            [OARouteProvider collectSegmentPointsFromGpx:_file points:points segmentEndPoints:endPoints selectedSegment:selectedSegment];
            self.points = points;
            _segmentEndPoints = endPoints;
        }
        if (points.count == 0)
        {
            for (OASRoute *rte in _file.routes)
            {
...
- (BOOL)shouldCalculateGpxRouteWithParams:(OARouteCalculationParams *)params
{
    if (params.gpxRoute != nil)
    {
        ...
        if (gpxApproximator != nil && ![params.gpxRoute.file isAttachedToRoads])
        {
            ...
            OASGpxFile *gpx = [approximationHelper approximateGpxSync:params.gpxRoute.file params:gpxApproximator];
